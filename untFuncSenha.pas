...
uses
  Windows, Messages, System.SysUtils, System.Types, System.UITypes,
  System.Classes, System.Variants, Graphics, Controls, Forms,
  Dialogs, StdCtrls, ExtCtrls, Grids, DBGrids, IBQuery,
  ActnList, CheckLst, Buttons, Data.DB, Model.Connection, Horse, Horse.Jhonson,
  Horse.Commons, System.JSON, Model.Usuario,
  Horse.Http, IdSSLOpenSSLHeaders, IdSSLOpenSSL, REST.Types, Vcl.ComCtrls;
...
uses untdmModule, Controller.Usuario, UnitDM;
...
var
  BaseURL: string;
 {
function CreateUser(Req: THorseRequest; Res: THorseResponse; Next: TProc): TCliente;
var
  User: TCliente;
  Response: IHTTPResponse;
begin
  User := Req.Body<TCliente>;

  Response := THorseClient.Post(BaseURL + '/usuario', TJSON.ObjectToJsonString(User), [TNetHeader.Create('Content-Type', 'application/json')]);

  if Response.StatusCode = 201 then
  begin
    User.ID_CLIENTE := TJSONObject.ParseJSONValue(Response.Content).GetValue<Integer>('id');
...
procedure TfrmFuncSenha.SpeedButton1Click(Sender: TObject);
   //Req: THorseRequest; Res: THorseResponse; Next: TProc
var
 PostUsuario : TCliente;
 nome, login, senha : String;

//LRequestBody: TJSONObject;
//LName, LLogin, LSenha: string;
    erro : string;
    verbo : TRESTRequestMethod;
begin

 // LRequestBody := TJSONObject.Create;//Req.Body<TJSONObject>;
 ...
 { PostUsuario := TCliente.Create;
  nomeusuario := TCliente.Create;
  loginusuario := TCliente.Create;
  senhausuario := TCliente.Create;
  nomeusuario.NOME := lbl_edtNome.Text;
  loginusuario.LOGIN := lbl_edtLogin.Text;
  senhausuario.SENHA := lbl_edtSenha.Text;     }
    ...
    begin
        verbo := rmPUT;
        idUsuario := StrToInt(lbl_edtCodigo.text);
    end;
...
procedure TfrmFuncSenha.SpeedButton2Click(Sender: TObject);
var
    json : string;
...
begin
  ...
  with dm do begin
      ...
      dm.ReqUsuarioGet.Execute;

      if dm.ReqUsuarioGet.Response.StatusCode  <> 200 then
      begin
          showmessage('Ocorreu um erro na consulta: ' +
          dm.ReqUsuarioGet.Response.StatusCode.ToString);
      end;

      try


          with dbgUsuario do begin
        ...
        { //lvUsuarios.Update;
          ...
          json := dm.ReqUsuarioGet.Response.JSONValue.ToString;
          ...
          //(arrayClientes.Get(i).GetValue<integer>('id', 0),
              AddClienteLV(arrayClientes.Get(i).GetValue<integer>('id', 0),
                           arrayClientes.Get(i).GetValue<string>('nome', ''),
                           arrayClientes.Get(i).GetValue<string>('login', ''));

          arrayClientes.DisposeOf;  }

      finally
         dm.ReqUsuarioGet.Destroy;
      end;
...
begin
 ...
 { RESTClient := TRESTClient.Create('http://api.example.com'); // Substitua pela URL da sua API
