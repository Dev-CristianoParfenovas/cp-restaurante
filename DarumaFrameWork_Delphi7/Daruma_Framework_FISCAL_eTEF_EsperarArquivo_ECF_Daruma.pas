...
procedure TFR_FISCAL_eTEF_EsperarArquivo_ECF_Daruma.EnviarClick(
  Sender: TObject);
var Str_Path_Arquivo_Resp_TEF, Str_TempoEspera :string;
begin
   Str_Path_Arquivo_Resp_TEF := Trim(edt_pathArquivoTEF.text);
   Str_TempoEspera:= Edt_Tempo.text;
   if RdgTravarTeclado.ItemIndex = 0 then
     Begin
        Int_Retorno := eTEF_EsperarArquivo_ECF_Daruma(Str_Path_Arquivo_Resp_TEF, StrToInt(Str_TempoEspera), true);
        FR_MenuImpressoraFiscal_Principal.DarumaFramework_Mostrar_Retorno(Int_Retorno);
     end
     else begin
        Int_Retorno := eTEF_EsperarArquivo_ECF_Daruma(Str_Path_Arquivo_Resp_TEF,StrToInt(Str_TempoEspera), false);
        FR_MenuImpressoraFiscal_Principal.DarumaFramework_Mostrar_Retorno(Int_Retorno);
     end;
end;
