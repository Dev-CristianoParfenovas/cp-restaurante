...
begin
       iRetorno:= eBuscarPortaVelocidade_MODEM_DarumaFramework();
        ...
        if (iRetorno = 1) Then
    begin
      Application.MessageBox('Método executado com sucesso.', 'Daruma DLL Framework', MB_OK + MB_ICONINFORMATION);
  end
  else
    begin
      Application.MessageBox(Pchar('Retorno do método= ' + IntToStr(iRetorno)), 'Daruma DLL Framework', MB_OK + MB_ICONERROR);
    end;
end;
...
begin
  ...
  if (iRetorno = 1) Then
    begin
      Application.MessageBox(pchar('Informação retornada = ' + Str_InfoEstendida), 'Daruma DLL Framework', MB_OK + MB_ICONINFORMATION);    end
  else
    begin
      Application.MessageBox('Erro ao retornar informação estendida!', 'Daruma DLL Framework', MB_OK + MB_ICONERROR);
    end;
end;
