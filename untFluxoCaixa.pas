...
uses
  ...
  Dialogs, Grids, DBGrids, StdCtrls, ExtCtrls, ComCtrls, IBQuery, Data.DB;
    ...
    (ibTempVendaLoja.FieldByName('VLRUNIT') as TFloatField).DisplayFormat := CasasDecimais('Produtos');
      ...
      while not ibTempVendaLoja.Eof do begin
        ...
        if TotalFluxoCaixa = 0 Then begin
          ...
          TotalFluxoCaixa := StrToFloat(StringReplace(FloatToStr(ibTempVendaLoja.FieldByName('VLRTOTAL').AsFloat),FormatSettings.ThousandSeparator,'',[rfreplaceAll]));

        end else begin

          TotalFluxo := StringReplace(FloatToStr(ibTempVendaLoja.FieldByName('VLRTOTAL').AsFloat),FormatSettings.ThousandSeparator,'',[rfReplaceAll]);

          TotalFluxoCaixa := TotalFluxoCaixa + StrtoFloat(StringReplace(TotalFluxo,FormatSettings.ThousandSeparator,'',[rfReplaceAll]));
           ...
           if (ibTempServicosVendas.FieldByName('PENDENTEPAGO').AsString = 'Pago')Then begin
             ...
             if ibTempServicosVendas.FieldByName('VLRTOTAL').AsInteger > 0 Then begin
               ...
               if TotalGeral = 0 Then begin
                 ...
                 TotalGeral := StrToFloat(StringReplace(FloatToStr(ibTempServicosVendas.FieldByName('VLRTOTAL').AsFloat),FormatSettings.ThousandSeparator,'',[rfReplaceAll]));

               end else begin

                 VlrGeral := (StringReplace(FloatToStr(ibTempServicosVendas.FieldByName('VLRTOTAL').AsFloat),FormatSettings.ThousandSeparator,'',[rfReplaceAll]));
          ...
          if (ibTempServicosVendas.FieldByName('PENDENTEPAGO').AsString = 'Pago')Then begin
            ...
            if ibTempServicosVendas.FieldByName('TIPOPAGTO').AsString = 'DINHEIRO' Then begin
              ...
              if TotalDinheiro = 0 Then begin
                ...
                TotalDinheiro := StrToFloat(StringReplace(FloatToStr(ibTempServicosVendas.FieldByName('VLRTOTAL').AsFloat),FormatSettings.ThousandSeparator,'',[rfReplaceAll]));

              end else begin

                VlrDinheiro := (StringReplace(FloatToStr(ibTempServicosVendas.FieldByName('VLRTOTAL').AsFloat),FormatSettings.ThousandSeparator,'',[rfReplaceAll]));
           ...
           if (ibTempServicosVendas.FieldByName('PENDENTEPAGO').AsString = 'Pago')Then begin
             ...
             if ibTempServicosVendas.FieldByName('TIPOPAGTO').AsString = 'CHEQUE' Then begin
               ...
               if TotalCheque = 0 Then begin
                 ...
                 TotalCheque := StrToFloat(StringReplace(FloatToStr(ibTempServicosVendas.FieldByName('VLRTOTAL').AsFloat),FormatSettings.ThousandSeparator,'',[rfReplaceAll]));

               end else begin

                 VlrCheque := (StringReplace(FloatToStr(ibTempServicosVendas.FieldByName('VLRTOTAL').AsFloat),FormatSettings.ThousandSeparator,'',[rfReplaceAll]));
           ...
           if (ibTempServicosVendas.FieldByName('PENDENTEPAGO').AsString = 'Pago')Then begin
             ...
             if ibTempServicosVendas.FieldByName('TIPOPAGTO').AsString = 'CHEQUE PRE' Then begin
               ...
               if TotalChequePre = 0 Then begin
                 ...
                 TotalChequePre := StrToFloat(StringReplace(FloatToStr(ibTempServicosVendas.FieldByName('VLRTOTAL').AsFloat),FormatSettings.ThousandSeparator,'',[rfReplaceAll]));

               end else begin

                 VlrChequePre := (StringReplace(FloatToStr(ibTempServicosVendas.FieldByName('VLRTOTAL').AsFloat),FormatSettings.ThousandSeparator,'',[rfReplaceAll]));
           ...
           if (ibTempServicosVendas.FieldByName('PENDENTEPAGO').AsString = 'Pago')Then begin
             ...
             if ibTempServicosVendas.FieldByName('TIPOPAGTO').AsString = 'CARTAO' Then begin
               ...
               if TotalCartao = 0 Then begin
                 ...
                 TotalCartao := StrToFloat(StringReplace(FloatToStr(ibTempServicosVendas.FieldByName('VLRTOTAL').AsFloat),FormatSettings.ThousandSeparator,'',[rfReplaceAll]));

               end else begin

                 VlrCartao := (StringReplace(FloatToStr(ibTempServicosVendas.FieldByName('VLRTOTAL').AsFloat),FormatSettings.ThousandSeparator,'',[rfReplaceAll]));

                 TotalCartao := TotalCartao + StrToFloat(StringReplace(VlrCartao,FormatSettings.ThousandSeparator,'',[rfReplaceAll]));
...
////////////////////////////////////////////////////////////////////////////////
                  ...
                  if (ibTempServicosVendas.FieldByName('PENDENTEPAGO').AsString = 'Pago')or(ibTempServicosVendas.FieldByName('PENDENTEPAGO').AsString = 'Pendente')Then begin
                   ...
                   if TotalDesconto = 0 Then begin
                     ...
                     TotalDesconto := StrToFloat(StringReplace(FloatToStr(ibTempServicosVendas.FieldByName('DESCONTO').AsFloat),FormatSettings.ThousandSeparator,'',[rfReplaceAll]));

                   end else begin

                     VlrDesconto := (StringReplace(FloatToStr(ibTempServicosVendas.FieldByName('DESCONTO').AsFloat),FormatSettings.ThousandSeparator,'',[rfReplaceAll]));

                     TotalDesconto := TotalDesconto + StrToFloat(StringReplace(VlrDesconto,FormatSettings.ThousandSeparator,'',[rfReplaceAll]));
...
////////////////////////////////////////////////////////////////////////////////
                 ...
                 if (ibTempServicosVendas.FieldByName('PENDENTEPAGO').AsString = 'Pago')or(ibTempServicosVendas.FieldByName('PENDENTEPAGO').AsString = 'Pendente')Then begin
                   ...
                   if TotalAcrescimo = 0 Then begin
                     ...
                     TotalAcrescimo := StrToFloat(StringReplace(FloatToStr(ibTempServicosVendas.FieldByName('ACRESCIMO').AsFloat),FormatSettings.ThousandSeparator,'',[rfReplaceAll]));

                   end else begin

                     VlrAcrescimo := (StringReplace(FloatToStr(ibTempServicosVendas.FieldByName('ACRESCIMO').AsFloat),FormatSettings.ThousandSeparator,'',[rfReplaceAll]));
...
////////////////////////////////////////////////////////////////////////////////
                 ...
                 if ibTempServicosVendas.FieldByName('PENDENTEPAGO').AsString = 'Pendente' Then begin
                     ...
                     if TotalPendente = 0 Then begin
                       ...
                       TotalPendente := StrToFloat(StringReplace(FloatToStr(ibTempServicosVendas.FieldByName('VLRTOTAL').AsFloat),FormatSettings.ThousandSeparator,'',[rfReplaceAll]));

                     end else begin

                       VlrPendente := (StringReplace(FloatToStr(ibTempServicosVendas.FieldByName('VLRTOTAL').AsFloat),FormatSettings.ThousandSeparator,'',[rfReplaceAll]));
                ...
                ibTempServicosVendas.Next;
                    ...
                    { ibPagtoPendente.Close;
                     ...
                     if TotalPagoPendente = 0 Then begin
                       ...
                       TotalPagoPendente := StrToFloat(StringReplace(FloattoStr(ibPagtoPendente.FieldByName('VLRPAGO').AsFloat),FormatSettings.ThousandSeparator,'',[rfReplaceAll]));

                     end else begin

                       VlrPagoPendente    := (StringReplace(FloatToStr(ibPagtoPendente.FieldByName('VLRPAGO').AsFloat),FormatSettings.ThousandSeparator,'',[rfReplaceAll]));
