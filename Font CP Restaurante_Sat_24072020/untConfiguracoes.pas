...
uses
  ...
  Dialogs, StdCtrls, BmsXPCheckBox, untClasse.VendasCX,
...
type
  TfrmConfiguracoes = class(TForm)
    ...
    gbSimplesConf: TGroupBox;
    ckSimSimplesConf: TBmsXPCheckBox;
    ckNaoSimplesConf: TBmsXPCheckBox;
    grbSatGeral: TGroupBox;
    ckSimSatGeral: TBmsXPCheckBox;
    ckNaoSatGeral: TBmsXPCheckBox;
    ...
    procedure ckSimSimplesConfCheck(Sender: TObject);
    procedure ckNaoSimplesConfCheck(Sender: TObject);
  private
...
////////////////////////////////////////////////////////////////////////////////
    ...
    if ibConfig.FieldByName('IMPRIMESIMPLESCONF').AsInteger = 1 Then begin

      ckSimSimplesConf.Checked := True;
      ckNaoSimplesConf.Checked := False;

    end else begin

      ckSimSimplesConf.Checked := False;
      ckNaoSimplesConf.Checked := True;
...
////////////////////////////////////////////////////////////////////////////////
    ...
    if ckSimSimplesConf.Checked Then begin

      intSimplesConf.IntCupomSimplesConf := 1;

    end;

    if ckNaoSimplesConf.Checked Then begin

      intSimplesConf.IntCupomSimplesConf := 0;

    end;

    if (ckSimSimplesConf.Checked = False)and(ckNaoSimplesConf.Checked = False) Then begin
  ...
  {  if (ckSimEstoque.Checked = False) and (ckNaoEstoque.Checked = False) Then begin
   ...
   if ConfirmaGravacao = 6 Then begin
      ...
      if ibConfig.RecordCount > 0 Then begin
        ...
        + 'IMPRIMESIMPLESCONF=''' + IntToStr(intSimplesConf.IntCupomSimplesConf) + ''','
        ...
        + 'LPT1,COM1,DESENHODIVISAO,CABCUPOMCOMPL,CAMINHOIMPRESSORA,SATGERAL,IMPRIMESIMPLESCONF,IMPRIMIRCUPOM) values '
        + '(''' + EstoqueM + ''','
        + ' ''' + ProdEstoque + ''','
        + ' ''' + ConfirmaFrase + ''','
        + ' ''' + CancelarItemCx + ''','
        ...
        + '''' + IntToStr(intSimplesConf.IntCupomSimplesConf) + ''','
        + ' ''' + ConfirmaCupom + ''')');
        ibConfig.ExecSQL;
        Commit(ibConfig);
...
procedure TfrmConfiguracoes.ckSimSimplesConfCheck(Sender: TObject);
begin
ckNaoSimplesConf.Checked := False;
end;

procedure TfrmConfiguracoes.ckNaoSimplesConfCheck(Sender: TObject);
begin
ckSimSimplesConf.Checked := False;
end;
